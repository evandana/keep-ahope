        ...
        .then(function(auth2) {
            ...
            if ( window._GOOGLE_CLOUD_AUTH2_.isSignedIn.get() ) {
                        ...
                        .then(result => {
                            if(result) {
                                ...
                                setCurrentUserAsHavingPermissions({
                                    ...
                                    fullName: linkedParseUser.attributes.fullName,
                                    uid: linkedParseUser.id,
                                });
                            } else {
                                // console.log('user is logged in but doesn\'t have permissions')
                                setCurrentUserAsNotHavingPermissions({
                                    email: linkedParseUser.attributes.email,
                                    fullName: linkedParseUser.attributes.fullName,
                                    uid: linkedParseUser.id,
                                });
                ...
                window._GOOGLE_CLOUD_AUTH2_.signIn().then(googleUser => {
                    ...
                    window._UI_STORE_.dispatch({ type: LOGIN_REQUEST })  // call self to restart flow (to be refactored for better pattern)
                })
                .catch(() => {
...
export const logoutRequest = () => {
    return window._GOOGLE_CLOUD_AUTH2_.signOut()
        .then( () => {
